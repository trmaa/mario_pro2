#include <set>
#include "game.hh"
#include "assert.hh"
#include "geometry.hh"
#include "utils.hh"

using namespace pro2;

Game::Game(int width, int height)
: mario_({width / 2, 150}, {Space, Left, Right}),
platforms_{
	Platform(100, 300, 200, 211),
	Platform(0, 200, 250, 261),
	Platform(250, 400, 150, 161),
},
finished_(false)
{
	assert(width > 0 && height > 0,
		"L'amplada i l'alcada del joc han de ser positives.");
	for (int i = 1; i < 1000000; i++) {
		platforms_.push_back(
			Platform(250 + i * 200, 400 + i * 200, 150, 161));
	}
	for (auto& plat : platforms_)
		plat_finder_.add(&plat);

	for (int j = 0; j < 20; j++)
		for (int i = 1; i < 5; i++)
			coins_.push_back(
					Coin({j*200 + 250 + i * 200/5, 130}));
	for (auto& coin : coins_)
		coin_finder_.add(&coin);
}

void Game::process_keys(pro2::Window& window) {
	if (window.is_key_down(Keys::Escape)) {
		finished_ = true;
		return;
	}

	if (window.is_key_down('P') && !window.was_key_pressed('P'))
		paused_ = !paused_;
}

void Game::update_objects(pro2::Window& window) {
	mario_.update(window, plat_finder_.query(window.camera_rect()));

	for (auto it = coins_.begin(); it != coins_.end();) {
		if (mario_.coliding(it->pos())) {
			it = coins_.erase(it);  // erase returns next valid iterator
			score_++;
		} else {
			it++;
		}

		it->move(tick_);
	}
}

void Game::update_camera(pro2::Window& window) {
	const Pt pos = mario_.pos();
	const Pt cam = window.camera_center();

	window.move_camera({pos.x-cam.x, pos.y-cam.y});
}

void Game::update(pro2::Window& window) {
	tick_++;

	process_keys(window);

	if (!paused_) {
		update_objects(window);
		update_camera(window);
	}
}

void Game::paint(pro2::Window& window) {
	window.clear(sky_blue);

	pro2::Rect test_rect = window.camera_rect();
	test_rect.left += 70;
	test_rect.top += 30;
	test_rect.right -= 70;
	test_rect.bottom -= 30;
	paint_rect(window, test_rect, 0x00ff00ff);

	for (auto p : plat_finder_.query(test_rect))
		p->paint(window);
	/*
	for (auto p : platforms_)
		p.paint(window);
	*/

	for (auto coin : coin_finder_.query(test_rect))
		coin->paint(window, tick_);

	mario_.paint(window);
}
